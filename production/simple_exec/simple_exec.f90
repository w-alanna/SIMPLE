...
use simple_commander_test_trpv_refine3D !NEW
use simple_commander_test_exp_refine3D !NEW
implicit none
...
type(test_bgal_refine3D_commander)          :: xbgal_refine3D
type(test_refine3D_trpv_commander)          :: xtrpv_refine3D !NEW
type(test_refine3D_exp_commander)           :: xexp_refine3D !NEW
...
select case(trim(prg))
    ...
    case( 'test_trpv_refine3D' )
        call xtrpv_refine3D%execute( cline ) !NEW!!!!!!
    case( 'test_exp_refine3D' )
        call xexp_refine3D%execute( cline ) !NEW!!!!!!
...
call simple_print_git_version('247400ff')
! end timer and print
rt_exec = toc(t0)
